...
def get_batch_on_this_tp_rank(data_iterator):
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        seq_len_in_cp = args.seq_length // cp_size
        tokens=torch.empty((args.micro_batch_size,seq_len_in_cp), dtype = torch.int64 , device = torch.cuda.current_device())
        labels=torch.empty((args.micro_batch_size,seq_len_in_cp), dtype = torch.int64 , device = torch.cuda.current_device())
        loss_mask=torch.empty((args.micro_batch_size,seq_len_in_cp), dtype = torch.float32 , device = torch.cuda.current_device())
        if args.create_attention_mask_in_dataloader:
            attention_mask=torch.empty(
                    (args.micro_batch_size,1,seq_len_in_cp,args.seq_length), dtype = torch.bool , device = torch.cuda.current_device()
                )
        else:
            attention_mask=None
        position_ids=torch.empty((args.micro_batch_size,seq_len_in_cp), dtype = torch.int64 , device = torch.cuda.current_device())
